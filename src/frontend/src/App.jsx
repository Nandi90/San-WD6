...
const BRKLogo=({size=48,full,customLogo})=>{const imgStyle={height:size,width:"auto",maxWidth:"100%",objectFit:"contain",display:"block",flexShrink:0};if(full&&customLogo)return <img src={customLogo} alt="Logo" style={imgStyle}/>;if(full)return <img src={BRK_LOGO} alt="BRK Bereitschaften" style={imgStyle}/>;return(<svg width={size} height={size} viewBox="0 0 100 100" fill="none" style={{flexShrink:0}}><rect x="35" y="5" width="30" height="90" rx="2" fill={C.rot}/><rect x="5" y="35" width="90" height="30" rx="2" fill={C.rot}/></svg>);};
...
export default function App(){
  ...
  React.useEffect(()=>{fetch("/public/logo").then(r=>r.ok?r.blob():null).then(b=>b?setPublicLogo(URL.createObjectURL(b)):null).catch(()=>{});},[]);
  // LOGIN
  if(!user)return(
    <div style={{minHeight:"100vh",background:C.hellgrau,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:FONT.sans}}>
      ...
      <div style={{width:440,padding:40}}>
        <div style={{textAlign:"center",marginBottom:32}}><BRKLogo size={80} full customLogo={publicLogo}/><h1 style={{margin:"12px 0 4px",fontSize:20,fontWeight:800,color:C.schwarz}}>SanWD Gefahrenanalyse</h1><p style={{margin:0,fontSize:13,color:C.dunkelgrau}}>Bayerisches Rotes Kreuz · Bereitschaften</p></div>
          ...
          <Card accent={C.rot}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                ...
                {smtpEnabled&&<button title="Angebot per E-Mail senden" onClick={async()=>{if(!currentEventId){toast("Bitte zuerst Vorgang speichern","warning");return;}await saveEvent();setMailModal(true);}} style={{width:36,height:36,borderRadius:8,border:"1px solid #ef9a9a",background:"#fff",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:18,transition:"all 0.2s"}} onMouseEnter={e=>e.currentTarget.style.background="#ffebee"} onMouseLeave={e=>e.currentTarget.style.background="#fff"}>✉️</button>}
