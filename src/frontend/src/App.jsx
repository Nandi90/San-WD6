            ...
            <div>
              ...
              <Card title="Veranstaltung" accent="#1a7a3a">
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 16px"}}>
                  ...
                  <AddressAutocomplete label="Adresse (Autocomplete + w3w)" value={event.adresse} onChange={v=>updateEvent("adresse",v)} onResult={s=>{updateEvent("coords",{lat:s.lat,lng:s.lng});if(s.w3w)updateEvent("w3w",s.w3w);}}/>
                </div>
          ...
          <Card accent={C.rot}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                ...
                {(pdfView==="angebot")&&<Btn onClick={async()=>{if(!currentEventId){alert("Bitte zuerst speichern");return;}try{const blob=await API.generateAngebotPDF(currentEventId,dayCalcs,totalCosts,activeDays);const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=(event.auftragsnr||"Angebot").replace(/[^a-zA-Z0-9_-]/g,"_")+"_Angebot.pdf";a.click();}catch(e){alert("Fehler: "+e.message);}}} icon="⬇️" variant="blue">PDF herunterladen</Btn>}
                {(pdfView==="aab")&&<Btn onClick={async()=>{if(!currentEventId){alert("Bitte zuerst speichern");return;}try{const blob=await API.generateAABPDF(currentEventId);const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=(event.auftragsnr||"AAB").replace(/[^a-zA-Z0-9_-]/g,"_")+"_AAB.pdf";a.click();}catch(e){alert("Fehler: "+e.message);}}} icon="⬇️" variant="blue">AAB herunterladen</Btn>}
                <Btn onClick={async()=>{if(!currentEventId){alert("Bitte zuerst Vorgang speichern");return;}await saveEvent();try{const blob=await API.generateMappePDF(currentEventId,dayCalcs,totalCosts,activeDays);const url=URL.createObjectURL(blob);const a=document.createElement("a");const nr=(event.auftragsnr||"").replace(/[^a-zA-Z0-9_-]/g,"_");const name=(event.name||"Veranstaltung").substring(0,30).replace(/ /g,"_");a.href=url;a.download=nr+"_"+name+"_Angebotsmappe.pdf";a.click();}catch(e){alert("Fehler bei Angebotsmappe: "+e.message);}}} icon="📦" variant="success">{mappePending?"Erstelle PDF...":"Angebotsmappe (PDF)"}</Btn>
              </div>
            </div>
          </Card>
