          ...
          <Card accent={C.rot}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:10}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                ...
                <Btn onClick={async()=>{if(!currentEventId){toast("Bitte zuerst Vorgang speichern","warning");return;}await saveEvent();try{const r=await API.syncToNextcloud(currentEventId,{dayCalcs,totalCosts,activeDays});if(r.success){toast("☁️ "+r.results.length+" Dateien in Nextcloud synchronisiert","success");setEvent(p=>({...p,nextcloudSync:{syncedAt:r.syncedAt,folder:r.folder,files:r.results.map(f=>f.file),syncedBy:user?.name}}));}else{toast("Sync fehlgeschlagen: "+(r.error||"Unbekannter Fehler"),"error");}}catch(e){toast("Nextcloud: "+e.message,"error");}}} icon="☁️" variant="blue" disabled={mappePending}>Nextcloud Sync</Btn>
                {event.nextcloudSync&&<div style={{marginTop:8,padding:"8px 12px",background:"#e3f2fd",border:"1px solid #90caf9",borderRadius:6,fontSize:11,color:"#1565c0",display:"flex",alignItems:"center",gap:8}}>
                  <span style={{fontSize:16}}>☁️</span>
                  <div><strong>Nextcloud synchronisiert</strong> · {event.nextcloudSync.syncedBy} · {event.nextcloudSync.syncedAt?.substring(0,10)}<br/><span style={{color:"#666",fontSize:10}}>{event.nextcloudSync.files?.length||0} Dateien · {event.nextcloudSync.folder}</span></div>
                </div>}
              </div>
            </div>
          </Card>
